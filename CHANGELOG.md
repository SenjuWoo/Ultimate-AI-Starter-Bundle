...
- One soul, one contract, per file: the whole Hermes home SOUL.md plus every `profiles\<name>\SOUL.md` get wired, not just the home copy. The live machine's duplication (the operating contract twice in the home and in the creative profile; four profiles carrying the contract with no soul at all) is fixed, and the gate now proves no-marker, no-duplicate, idempotent wiring by running the writer twice in a probe.
- Retire `stealth/ox-alpha`: delisted upstream (checked against the live OpenRouter model list). Gone from the shipped config, the alias ladder, the vision chain and the README. The migrator sweeps the pack's own dead references - an alias or vision fallback pointing at a retired id - from every profile, matched exactly, and leaves anything the pack did not write alone.
- Refresh eight stale catalog pins: context7 4.1.0 → 4.1.1 (capability record re-measured: 2 tools, 4,870 bytes, both keyless), playwright-mcp 0.0.80 → 0.0.81, playwright-cli 0.1.19 → 0.1.20 (pins Playwright 1.64.0-alpha-2026-09-14), super-mcp-router 2.7.6 → 2.8.2, omni 0.7.8 → 0.7.9, unity-mcp CLI 0.90.0 → 0.91.0 (IvanMurzak lineage), github-mcp-server 1.12.1 → 1.12.2 and codebase-memory 0.10.8 → 0.11.0. Both offline assets were re-fetched from upstream and verified against the releases' own checksum files before the manifest was rebound.
- Held deliberately, with reasons: houseCARL stays at 1.9.0 (the 2.x tool surface is what the Lean filter set and the measured schema numbers are bound to), ponytail at 4.9.0 and rtk at 0.47.0 (offline payloads and the shell hook contract need re-validation), codebase-memory's tool-surface numbers remain recorded at 0.10.8, and impeccable stays at CLI 3.6.1 / skill 4.1.3 under its catalog `compatibility_hold` (upstream moved to a platform engine with first-use downloads).
- **169 canonical skills total**, no new dependency, no new MCP server.
...
- Replace stale universal cache prices, RTK saving claims and unsafe CI polling guidance with measured, host-specific policy.
...
- Fixed the default installer selecting upstream RTK 0.48.0 while its catalog and safe hook require the measured 0.47.0. Installer and component updater now resolve RTK's exact catalog tag and honor explicit compatibility pins for other release assets.
...
- Hardened Impeccable's local live server: poll and lease retention are capped at ten minutes, literal attribute markers are regex-escaped, malformed closing tags with bogus tails cannot leak script/style text into analysis, and HTML evidence entities decode exactly once. Proven classification-only CodeQL findings are documented as such.
...
- **Editing a canonical skill shipped nothing, and the gate said green.**
  ...
  `1-TAILORED-PROVIDER-TREES`, generated by `TOOLS/fanout_providers.py`. Two
  skills were corrected, 104 contracts passed, the pack gate passed, the
  installer reported "skills installed" for all five providers -- and every
  provider copy was still yesterday's file. One existing contract byte-compares
  only a short list of skills; another compares only the *set of names*. Both
  edited skills fell through the gap. The new contract compares **all 164
  skills across all five trees**, normalising only the `provider:` tag and line
  endings, and its failure message names the fanout command. Same shape as the
  context7 pin: a value in two places, and the second copy is what ships.
...
- **An unread version string is still a claim.** `PROFILES.json` declared
  ...
  on the next drift.
- 105 contracts, green; all eight new ones falsified before shipping.
